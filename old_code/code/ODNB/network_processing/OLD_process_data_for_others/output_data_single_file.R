#@S Output data to csv format, using name_r,name_c,confidence format. 
...
for(j in 1:(ncol(M_adj)-1)) {
  ...
  to_add = cbind(names_adj$Entity.Name[j], names_adj$Entity.Name[lines], M_adj[j,lines])
  if (ncol(to_add) < 3) {
...
write.csv(res, file = "named_conf_ests.csv", row.names = FALSE)
